...
def main():
    ...
    plot_results = False
    if plot_results:
        jacc_ = np.load("jacc.npy")
        loss_ = np.load("loss.npy")
        dice_ = np.load("dice.npy")
    ...
    if train_unet:
        ...
        np.save("jacc.npy", np.array(jacc_list))
        np.save("dice.npy", np.array(dice_list))
        np.save("loss.npy", np.array(loss_list))

        torch.save(u_net.state_dict(), str(num_epochs) + "ep_" + str(batch_size) + "bs.pth")
    else:

        u_net.load_state_dict(torch.load("./50ep_1bs.pth"))
